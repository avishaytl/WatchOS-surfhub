...
class SettingsStore(context: Context) : KeyValueStore {
    ...
    private val prefs: SharedPreferences =
        context.applicationContext.getSharedPreferences("spoteq_settings", Context.MODE_PRIVATE)
    ...
    /** Minimum recorded jump height. The only valid choices are 1.5/2/3/4 m. */
    var minJumpHeightMeters: Double
        get() {
            val value = JumpHeightSetting.decodePersisted(prefs.all[JumpHeightSetting.PREFERENCE_KEY])
            val canonical = JumpHeightSetting.encodePersisted(value)
            if (prefs.all[JumpHeightSetting.PREFERENCE_KEY] != canonical) {
                prefs.edit().putLong(JumpHeightSetting.PREFERENCE_KEY, canonical).apply()
            }
            return value
        }
        set(v) {
            prefs.edit()
                .putLong(JumpHeightSetting.PREFERENCE_KEY, JumpHeightSetting.encodePersisted(v))
                .apply()
        }
