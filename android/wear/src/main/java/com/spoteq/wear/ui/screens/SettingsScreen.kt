...
import com.spoteq.wear.ui.theme.themeColor
...
fun SettingsScreen(vm: SessionManager, nav: NavController) {
    ...
    ScalingLazyColumn(
        ...
        // Minimum jump height. This is always configured in metres because it
        // is a detector threshold, independently of the display-units choice.
        item { SectionTitle(context.getString(R.string.settings_min_jump_height)) }
        items(JumpHeightSetting.OPTIONS_METERS.size) { i ->
            val height = JumpHeightSetting.OPTIONS_METERS[i]
            val number = if (height % 1.0 == 0.0) height.toInt().toString() else height.toString()
            ChipRow(
                context.getString(R.string.settings_jump_height_option, number),
                selColors(settings.minJumpHeightMeters == height),
            ) { settings.minJumpHeightMeters = height }
        }
