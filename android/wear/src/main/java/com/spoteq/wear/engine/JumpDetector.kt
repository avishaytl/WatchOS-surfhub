...
class JumpDetector(
    ...
    internal var configuredMinJumpHeightMeters: Double = JumpHeightSetting.DEFAULT_METERS
        private set

    init {
        buildSession(DetectionMode.STANDARD, JumpHeightSetting.DEFAULT_METERS)
    }

    fun reset(
        mode: DetectionMode = DetectionMode.STANDARD,
        minJumpHeightMeters: Double = JumpHeightSetting.DEFAULT_METERS,
    ) {
        this.mode = mode
        buildSession(mode, minJumpHeightMeters)
    ...
    private fun buildSession(mode: DetectionMode, minJumpHeightMeters: Double) {
        configuredMinJumpHeightMeters = JumpHeightSetting.normalize(minJumpHeightMeters)
        val cfg = KitesurfJumpEngineV7.Config().apply {
            ...
            maxAirTimeSec = 6.50
            this.minJumpHeightMeters = configuredMinJumpHeightMeters
