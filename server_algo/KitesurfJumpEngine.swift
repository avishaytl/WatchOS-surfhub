...
struct JumpResult {
    ...
    let airTimeSeconds: Double          // PHYSICAL spike-to-spike (drives height)
    let displayedAirtimeSeconds: Double // trimmed to match the reference app (UI/upload)
...
final class KitesurfJumpEngineV7 {
    ...
    struct Config {
        ...
        // Grounded in real wrist logs (log1/log2): the take-off pop reads
        // ≈1.8–2.3 g with only a WEAK gyro (~0.3 rad/s), and the adaptive
        // median+Kσ floor balloons during chop and HIDES real take-offs — so
        // releaseSigmaK is small and a fixed releaseFloorG carries the gate.
        var releaseSigmaK           = 1.5
        var releaseFloorG           = 1.70
        var landingSpikeG           = 1.40   // hard-landing impact (g)
        // FIRST water-contact landing: a board slap can be modest in accel but
        // the wrist-rotation gyro burst is the tell. Stopping at the first
        // contact keeps two consecutive jumps from merging into one window.
        var landingContactG         = 1.15   // modest accel at water contact (g)
        var landingContactGyro      = 2.0    // wrist-rotation burst at contact (rad/s)
        var settleTolG              = 0.35   // |a| within this of ride-mean = "settled"
        var settleSamplesNeeded     = 6      // of last 8 → soft landing
        var minAirTimeSec           = 2.0    // real kite big-airs run 4–6 s; reject pops
        var maxAirTimeSec           = 6.5    // KINEMATIC watchdog (see height notes)
        var minJumpHeightMeters     = 1.5    // match the reference app's ≥1.5 m display gate
        ...
        // Loose final guard only (Big Air records exceed 45 m) — the drift filter
        // is the valley test, NOT this cap.
        var maxPlausibleHeightM     = 50.0
        // Reference-validated (Surfr): solving ½·g·(f·airtime)² against the
        // reference + pro Big-Air points gives f = 0.139–0.146 (0.143 minimax).
        // The kite holds the rider up, so the ascent is only ≈14% of the
        // spike-to-spike airtime.
        var symmetricAscentFraction = 0.143
        // Reference apps report a SHORTER airtime than the physical spike-to-spike
        // flight (they trim the kite-glide tail). Applied only to the DISPLAYED
        // airtime in toJumpEvent — the internal airTime drives the height model.
        var displayedAirtimeScale   = 0.73
    ...
    func process(_ rawSamples: [SensorSample],
        ...
        // ── LANDING: FIRST water contact (spike + gyro burst) OR baro recovery OR watchdog ──
        // The kite-glide "settle" path is intentionally REMOVED: the kite settles
        // the wrist MID-flight, which would cut the airtime (and the rise-time
        // height) far too short. Stopping at the FIRST water contact keeps two
        // consecutive jumps from merging into one over-long window.
        ...
        while i < n && (i - t0) <= maxAS {
            ...
            if air >= cfg.minAirTimeSec {
                // (a) FIRST water contact: board slap + wrist-rotation burst, or a hard impact.
                if (accMag[i] >= cfg.landingContactG && gyroMag[i] >= cfg.landingContactGyro)
                    || (accMag[i] >= cfg.landingSpikeG && gyroMag[i] >= 1.0) {
        ...
        // A 20 m+ Big Air has ~15 s of hangtime, far beyond the kinematic
        // watchdog (6.5 s), so the landing loop times out mid-flight before the
        // pressure apex. The KINEMATIC height MUST stay capped (rise-time on a
        // long airtime explodes), but the BAROMETER sees the whole arc — so on a
        // timeout WITH a developing dip, scan baro-only up to 30 s for the true
        // pressure minimum + recovery, WITHOUT extending the kinematic airTime.
        var baroApexIdx = tl
        var baroLandIdx = tl
        if haveBaro && landingKind == .timeout {
            let baroHorizon = min(n - 1, t0 + Int(30.0 / dt))
            var mp = Double.greatestFiniteMagnitude
            for k in t0...baroHorizon {
                jumpMinP = min(jumpMinP, baroSmooth[k])
                if baroSmooth[k] < mp { mp = baroSmooth[k]; baroApexIdx = k }
            }
            let dipE = baselineP - mp
            if dipE > cfg.baroNoiseFloorHPa {
                let recover = max(dipE * 0.5, cfg.baroNoiseFloorHPa)
                for k in baroApexIdx...baroHorizon where baroSmooth[k] >= baselineP - recover {
                    baroLandIdx = k; break
                }
            }
        }
        ...
        // ── KINEMATIC HEIGHT (KITE-AWARE rise-time) ─────────────────
        // h = ½·g·t_rise², t_rise = symmetricAscentFraction·airtime. Apex from the
        // baro minimum when present, clamped to the ascent boundary (the kite
        // lifts AFTER the apex, pulling the baro minimum late).
        ...
        if haveBaro && dP > cfg.baroNoiseFloorHPa {
            ...
            for k in t0...tl where baroSmooth[k] < mp { mp = baroSmooth[k]; minIdx = k }
            let tr = Double(minIdx - t0) * dt
            let trClamped = min(tr, airTimeSec * cfg.symmetricAscentFraction)
            if tr > 0.1 && tr < airTimeSec - 0.05 { tRise = trClamped }
        }
        let riseH = cfg.kinematicCalibration * 0.5 * K.g * tRise * tRise

        // ── PHYSICAL CONSISTENCY GATE (drift-rejecting, Big-Air-safe) ──
        // baroH is believable only if BOTH hold:
        //  (1) AIRTIME SUPPORTS IT — baroH ≤ free-fall ceiling g·t²/8 of the
        //      flight time the baro actually saw (t0 → baro landing, not the
        //      watchdog-capped kinematic airtime). Big Air passes; a big dP on a
        //      short airtime fails.
        //  (2) PRESSURE RECOVERS (valley) — the minimum is not at the window's
        //      end and the pressure climbs back ≥50% of the dip (or ≥15% if the
        //      minimum is clearly past, for a watchdog-cut Big Air). A monotone
        //      drift (min at the end) fails this — this is what rejects the 13 m
        //      baro-drift readings.
        let baroFlightSec = Double(baroLandIdx - t0) * dt
        let symCeiling = (K.g * baroFlightSec * baroFlightSec / 8.0) * cfg.airtimeCeilingTolerance
        var baroRecovers = true
        if haveBaro && dP > cfg.baroNoiseFloorHPa {
            let endP = baroSmooth[baroLandIdx]
            let recovered = endP - jumpMinP
            let valleyTurned = baroApexIdx < baroLandIdx - Int((0.2 / dt).rounded())
            baroRecovers = recovered >= 0.5 * dP || (valleyTurned && recovered >= 0.15 * dP)
        }
        let baroConsistent = baroH > 0 && baroH <= symCeiling && baroRecovers
        ...
        if baroConsistent {
            ...
            let baroClamped = min(baroH, symCeiling)
        ...
        // take-off speed over the BALLISTIC flight time implied by the measured
        // height (2·√(2h/g)), capped by the real airtime. The kite glide does NOT
        // add proportional horizontal travel, so using the full airtime here
        // over-reads distance ~2–3×.
        let ballisticFlightSec = min(airTimeSec, 2.0 * (2.0 * fusedH / K.g).squareRoot())
        let jumpDistSpeedTime = jumpDistGPS ?? distanceSpeedTime(samples, t0: t0, airTimeSec: ballisticFlightSec)
        ...
        return JumpResult(
            jumpHeightMeters:      r2(DSP.clamp(fusedH, 0, cfg.maxPlausibleHeightM)),
...
final class KitesurfSession {
    ...
    private let takeoffReleaseFloorG = 1.70   // matches V7 release floor (adaptive on top)
    private let releaseSigmaK       = 1.5
    private let minAirSec           = 2.0     // reject pops; real kite jumps ≥ ~4 s
    private let maxAirborneSec      = 6.5     // kinematic watchdog (matches V7 cfg)
    private let refractorySec       = 1.0     // no re-trigger right after landing
    ...
    /// Release spike: fixed-floor-dominated (the adaptive μ+Kσ term balloons
    /// during chop and HIDES real take-offs), gated by a WEAK gyro floor — the
    /// wrist pop's gyro is small (~0.3 rad/s), not the airborne-quality threshold.
    private func isReleaseSpike(_ s: SensorSample) -> Bool {
        let a = s.accelMagG
        guard a >= takeoffReleaseFloorG else { return false }
        guard rideWindow.count > 10 else { return a >= takeoffReleaseFloorG && s.gyroMag >= 0.3 }
        ...
        return a >= thr && s.gyroMag >= 0.3
    }

    /// Landing = FIRST water contact: a board slap (modest accel) WITH a wrist-
    /// rotation gyro burst, or a hard impact. The settle path is removed — the
    /// kite settles the wrist mid-flight and would cut the airtime short, and
    /// stopping at the first contact keeps two chained jumps from merging.
    private func landingDetected(_ s: SensorSample, air: Double) -> Bool {
        // (a) first water contact: board slap + gyro burst, or hard impact.
        if (s.accelMagG >= detectorLandingContactG && s.gyroMag >= detectorLandingContactGyro)
            || (s.accelMagG >= detectorLandingSpikeG && s.gyroMag >= 1.0) { return true }
    ...
    private let detectorLandingSpikeG    = 1.40
    private let detectorLandingContactG  = 1.15
    private let detectorLandingContactGyro = 2.0
    private let detectorNoiseFloorHPa    = 0.03
