...
export interface JumpEngineV7Params {
  ...
  settleSamplesNeeded: number; // of last 8 → soft landing
...
export const DEFAULT_V7_PARAMS: JumpEngineV7Params = {
  baroMedianHalfWindow: 7,
  baroLowPassAlpha1: 0.30,
  baroLowPassAlpha2: 0.15,
  ...
  maxAirTimeSec: 6.5,
  minJumpHeightM: 1.5,   // ignore pop / wave-chop hops; match the reference app's ≥1.5 m display gate
  gyroQualityThreshold: 8.0,
  gyroIsDegPerSec: false,
  kinematicCalibration: 1.0,
...
export type LandingKind = 'hardImpact' | 'baroRecovery' | 'settle' | 'timeout';
...
export function processJumpBuffer(
  ...
  if (t0HintMapped != null && t0HintMapped > 1 && t0HintMapped < n - 4) {
    ...
    for (let i = 2; i < n - 4; i++) {
      ...
      if (gyroAhead >= params.gyroQualityThreshold * 0.4 ||
  ...
  for (let i = t0 + 1; i < n && i - t0 <= maxAS; i++) {
    ...
    // rotation burst, or a hard impact. Stopping at the first contact keeps two
    // chained jumps from merging. The settle path is intentionally removed (the
    // kite settles the wrist mid-flight and would cut the airtime/height short).
    if ((accMag[i]! >= params.landingContactG && gyroOmega[i]! >= params.landingContactGyro) ||
        (accMag[i]! >= params.landingSpikeG && gyroOmega[i]! >= 1.0)) {
      tl = i; landingKind = 'hardImpact'; break;
    }
    // Baro recovery only ever fires on big-kite jumps that clear the baro noise
    // floor; harmless no-op for the common case where baro can't see the jump.
    if (haveBaro) {
      const drop = baselineP - jumpMinP;
      if (drop > params.baroNoiseFloorHPa) {
        const recover = Math.max(drop * 0.08, params.baroNoiseFloorHPa);
        if (baroSmooth[i]! >= baselineP - recover) { tl = i; landingKind = 'baroRecovery'; break; }
      }
    }
  }
...
export function detectJumpsV7(
  ...
  for (let i = 0; i < samples.length; i++) {
    ...
    if (state === 'riding') {
      ...
      // accel spike but only a WEAK gyro — gate on a low gyro floor (0.3 rad/s),
      ...
      // over-long window. A hard impact (big accel) also counts. The settle path
      // is disabled — the kite settles the wrist mid-flight and would cut airtime.
      let landed = false;
      if (air >= params.minAirTimeSec &&
          ((a >= params.landingContactG && w >= params.landingContactGyro) ||
           (a >= params.landingSpikeG && w >= 1.0))) {
        landed = true;
      }
      if (landed || air > params.maxAirTimeSec) {
        if (landed) {
          // capture buffer: preTail before takeoff … postTail after landing
          const lo = Math.max(0, takeoffIdx - preTailN);
          const hi = Math.min(samples.length - 1, i + postTailN);
          const buf = samples.slice(lo, hi + 1);
          const hint = takeoffIdx - lo;
          const r = processJumpBuffer(buf, maxSpeed, params, hint);
