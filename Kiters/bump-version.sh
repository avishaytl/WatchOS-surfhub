...
if [ $# -eq 0 ]; then
  echo "📱  Kiters v${current_version} (${current_build})"
  exit 0
fi
...
if [ "$1" = "--bump-build" ]; then
  ...
  echo "📱  Kiters v${current_version} (${current_build}) → v${current_version} (${new_build})"
  exit 0
fi
...
echo "📱  Kiters v${current_version} (${current_build}) → v${new_version} (${new_build})"
