...
//       50 Hz CoreMotion ─┐
//                         ├─► KitesurfSessionV10.onSample (streaming FSM)
//       1 Hz CoreLocation ┘            │ on landing
//                                      ▼
//                         KitesurfJumpEngineV10.process (offline, <10 ms)
//                                      ▼
...
// ================================================================
final class KitesurfJumpEngineV10 {
    ...
    struct Config {
        ...
        var hardLandingMinAirTimeSec = 3.00  // avoid board chop ending kite jumps at ~2s
        var baroRecoveryMinAirTimeSec = 3.00
        var settleTolG              = 0.35   // |a| within this of ride-mean = "settled"
        var settleSamplesNeeded     = 6      // of last 8 → soft landing
        var settleMinAirTimeSec     = 3.50
    ...
    func process(_ rawSamples: [SensorSampleV10],
                 takeoffHint t0HintRaw: Int? = nil,
                 maxSessionSpeedMS: Double) -> JumpResultV10? {
        ...
        if haveBaro {
            ...
            let m  = DSPV10.medianFilter(filled, halfWindow: cfg.baroMedianHalfWindow)
            let p1 = DSPV10.lowPass(m,  alpha: cfg.baroLowPassAlpha1)
            baroSmooth = DSPV10.lowPass(p1, alpha: cfg.baroLowPassAlpha2)
        } else {
        ...
        let rideMeanA  = DSPV10.median(Array(accMag[0..<baseWindow]))
        let rideStdA   = DSPV10.std(accMag[0..<baseWindow])
        let baselineP  = haveBaro ? DSPV10.median(Array(baroSmooth[0..<baseWindow])) : 0
        ...
        var tl = -1
        var landingKind: JumpResultV10.LandingKind = .timeout
        ...
        while i < n && (i - t0) <= maxAS {
            ...
            if air >= cfg.minAirTimeSec {
                // (a) hard impact
                if air >= cfg.hardLandingMinAirTimeSec, accMag[i] >= cfg.landingSpikeG {
                    tl = i; landingKind = .hardImpact; break
                }
                // (b) baro recovery (only meaningful if a real drop occurred)
                if haveBaro {
                    let drop = baselineP - jumpMinP
                    if air >= cfg.baroRecoveryMinAirTimeSec, drop > cfg.baroNoiseFloorHPa {
                ...
                if abs(accMag[i] - rideMeanA) < cfg.settleTolG && gyroMag[i] < cfg.gyroQualityThreshold {
                    settleRun += 1
                    if air >= cfg.settleMinAirTimeSec, settleRun >= cfg.settleSamplesNeeded && (i - t0) > minAS {
        ...
        let baroRange = haveBaro ? max(0, (baroSmooth[t0...tl].max() ?? baselineP) - (baroSmooth[t0...tl].min() ?? baselineP)) : 0
        let baselineDrop = haveBaro ? max(0, baselineP - jumpMinP) : 0
        let dP    = max(baselineDrop, baroRange)
        let baroH = dP > cfg.baroNoiseFloorHPa ? dP * K10.p2m : 0
        ...
        let symCeilingH = K10.g * airTimeSec * airTimeSec / 8.0
        ...
        // ── HEIGHT SELECTION ───────────────────────────────────────
        let heightM: Double
        let source: JumpResultV10.HeightSource
        if baroConsistent {
            let baroTrust = DSPV10.clamp((dP - cfg.baroTrustLoHPa) / (cfg.baroTrustHiHPa - cfg.baroTrustLoHPa), 0, 1)
        ...
        let rotations = Int((totalRad / K10.twoPi).rounded(.down))

        // ── GYRO QUALITY (for confidence) ─────────────────────────
        let gyroQ = gyroMag.map { max(0, 1 - $0 / (cfg.gyroQualityThreshold * 2.5)) }
        let avgGyroQ = DSPV10.mean(gyroQ[t0...tl])
        let peakGyro = peakGyroForHeight
        ...
        conf = DSPV10.clamp(conf, 0, 1)

        func r2(_ v: Double) -> Double { (v * 100).rounded() / 100 }
        func r1(_ v: Double) -> Double { (v * 10).rounded() / 10 }

        return JumpResultV10(
            jumpHeightMeters:      r2(DSPV10.clamp(fusedH, 0, 25)),
            ...
            maxSessionSpeedKnots:  r1(maxSessionSpeedMS * K10.ms2kn),
            maxSessionSpeedKmh:    r1(maxSessionSpeedMS * K10.ms2kmh),
    ...
    private func integrateApex(_ s: [SensorSampleV10], t0: Int, tl: Int, dt: Double) -> (Double?, Double) {
        // Bias = median of (gravity-projected accel) over quiet pre-jump, expected ≈ 0 g.
        let vert: [Double] = s.map { smp in
            let (gx, gy, gz) = DSPV10.normalize3(smp.gravX, smp.gravY, smp.gravZ)
            // Project the (gravity-removed) accel onto the up axis (−gravity). Units: g.
            return -DSPV10.dot3(smp.ax, smp.ay, smp.az, gx, gy, gz)
        }
        var bias = 0.0
        if cfg.enableIMUBiasCorrection, t0 > 3 { bias = DSPV10.median(Array(vert[0..<t0])) }
        var v = 0.0, prevV = 0.0, elapsed = 0.0, peakV = 0.0, apex: Double? = nil
        for k in (t0 + 1)...tl {
            let aMps2 = (vert[k] - bias) * K10.g     // vertical accel in m/s²
            prevV = v; v += aMps2 * dt; elapsed += dt
            peakV = max(peakV, v)
            if apex == nil, prevV > 0.05, v <= 0.05 {
                let frac = DSPV10.clamp(prevV / (prevV - v + 1e-9), 0, 1)
    ...
    private func distanceSpeedTime(_ s: [SensorSampleV10], t0: Int, airTimeSec: Double) -> Double? {
        let tT = s[t0].t
    ...
    private func distanceGPS(_ s: [SensorSampleV10], t0: Int, tl: Int) -> Double? {
        ...
        return (GPSUtilV10.haversine(nearT.lat, nearT.lon, nearL.lat, nearL.lon) * 10).rounded() / 10
    }

    // ── Helpers ──────────────────────────────────────────────────
    private func willBaroDrop(_ baro: [Double], from i: Int, baseline: Double) -> Bool {
        let end = min(baro.count - 1, i + Int(2.0 / K10.dt))
    ...
    private func resolveGyroScale(_ s: [SensorSampleV10]) -> Double {
        if let forced = cfg.gyroIsDegPerSec { return forced ? K10.deg2rad : 1.0 }
        let mags = s.map { $0.gyroMag }
        return DSPV10.median(mags) > 10 ? K10.deg2rad : 1.0
    }
    ...
    static func dedupeByTime(_ s: [SensorSampleV10]) -> ([SensorSampleV10], [Int]) {
        var out = [SensorSampleV10](); out.reserveCapacity(s.count)
    ...
    private func estimateDt(_ s: [SensorSampleV10]) -> Double {
        guard s.count >= 6 else { return K10.dt }
        var diffs = [Double]()
        for i in 1..<min(40, s.count) {
            let d = s[i].t - s[i - 1].t
            if d > 0.005, d < 0.5 { diffs.append(d) }
        }
        return diffs.count > 3 ? DSPV10.median(diffs) : K10.dt
    }
}
...
// ================================================================
final class CircularBufferV10<T> {
...
final class KitesurfSessionV10 {
    ...
    private var dt = K10.dt
    private var hz = K10.sampleRate
    ...
    private var rollingBuffer: CircularBufferV10<SensorSampleV10>
    private var jumpBuffer = [SensorSampleV10]()
    private let detector: KitesurfJumpEngineV10
    ...
    // ── Output ───────────────────────────────────────────────────
    var onJumpDetected: ((JumpResultV10) -> Void)?
    var onSpeedUpdate:  ((_ knots: Double) -> Void)?
    var onStateChange:  ((State) -> Void)?

    init(detectorConfig: KitesurfJumpEngineV10.Config = .default,
         refractorySec: Double = 1.0,
         synchronousAnalysis: Bool = false) {
        self.detector = KitesurfJumpEngineV10(config: detectorConfig)
        ...
        self.detectorHardLandingMinAirSec = detectorConfig.hardLandingMinAirTimeSec
    ...
    /// Soft kite jumps in log2 can be barometer/gyro-visible with very little
    /// wrist acceleration. Trigger when pressure recovers sharply from a recent
    /// low while the rider is moving; this catches the Surfr-like soft events
    /// without lowering the acceleration floor for every bit of chop.
    private func isBaroRecoveryJumpSignal(_ s: SensorSampleV10) -> Bool {
        guard let p = s.baro, baselineReady, maxSessionSpeedMS >= 4.0 else { return false }
        let recent = rollingBuffer.last(samples(8.0)).compactMap { smp -> (t: Double, p: Double)? in
            guard let bp = smp.baro else { return nil }
            return (smp.t, bp)
        }
        guard recent.count >= 3,
              let minPoint = recent.min(by: { $0.p < $1.p }) else { return false }
        let recovery = p - minPoint.p
        guard recovery >= 0.28,
              s.t - minPoint.t >= 1.0,
              s.t - minPoint.t <= 8.0 else { return false }
        return s.gyroMag >= 0.8 || s.accelMagG >= takeoffReleaseFloorG * 0.25
    }

    /// Landing: hard impact OR (significant) baro recovery OR sustained settle.
    private func landingDetected(_ s: SensorSampleV10, air: Double) -> Bool {
        // (a) hard impact
        if air >= detectorHardLandingMinAirSec, s.accelMagG >= detectorLandingSpikeG { return true }

        // (b) baro recovery — only when a real drop occurred (clears noise floor).
        if let p = s.baro {
            let drop = baselineAtTakeoff - jumpMinPressure
            if air >= detectorBaroRecoveryMinAirSec, drop > detectorNoiseFloorHPa {
        ...
        if abs(s.accelMagG - rideMean) < 0.35 && s.gyroMag < 8.0 {
            settleRun += 1
            if air >= detectorSettleMinAirSec, settleRun >= detectorSettleSamplesNeeded { return true }
        } else {
            settleRun = 0
        }
        return false
    ...
    private let detectorHardLandingMinAirSec: Double
    private let detectorBaroRecoveryMinAirSec: Double
    private let detectorSettleMinAirSec: Double
    private let detectorSettleSamplesNeeded: Int
    ...
    private func lockRate(_ t: Double) {
        ...
        if let prev = lastT {
            ...
            if rateSamples.count >= 20 {
                ...
                if want != rollingBuffer.capacity { rollingBuffer = CircularBufferV10<SensorSampleV10>(capacity: want) }
            }
        }
        lastT = t
    }
    ...
    private func median(_ a: [Double]) -> Double { DSPV10.median(a) }
    private func mean(_ a: [Double]) -> Double { DSPV10.mean(a) }
...
/*
 ...
 final class WorkoutController: NSObject, CLLocationManagerDelegate {
     ...
     private let session  = KitesurfSessionV10()
