...
This document covers local development setup with Docker, deployment strategies, and DevOps best practices for the iSurf backend.
...
services:
  ...
  postgres:
    ...
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U isurf"]
      interval: 10s
      timeout: 5s
      retries: 5
...
CORS_ORIGIN=http://localhost:3000,isurf://
...
CORS_ORIGIN=https://isurf.app,isurf://
...
echo "🚀 Starting iSurf development environment..."
...
docker-compose exec postgres psql -U isurf -c "DROP DATABASE IF EXISTS isurf;"
docker-compose exec postgres psql -U isurf -c "CREATE DATABASE isurf;"
...
```json
{
  ...
  "containerDefinitions": [
    {
      "name": "api",
      "image": "<AWS_ACCOUNT_ID>.dkr.ecr.<REGION>.amazonaws.com/isurf-api:latest",
      "portMappings": [
        {
          "containerPort": 3000,
          "protocol": "tcp"
      ...
      "secrets": [
        {
          "name": "DB_PASSWORD",
          "valueFrom": "arn:aws:secretsmanager:region:account:secret:isurf/db/password"
        },
        {
          "name": "JWT_SECRET",
          "valueFrom": "arn:aws:secretsmanager:region:account:secret:isurf/jwt/secret"
        }
      ],
      "logConfiguration": {
        "logDriver": "awslogs",
        "options": {
          "awslogs-group": "/ecs/isurf-api",
...
env:
  ...
  ECR_REPOSITORY: isurf-api
  ECS_SERVICE: isurf-api-service
  ECS_CLUSTER: isurf-cluster
...
BACKUP_FILE="$BACKUP_DIR/isurf_$TIMESTAMP.sql.gz"

# Create backup
docker-compose exec -T postgres pg_dump -U isurf isurf | gzip > "$BACKUP_FILE"
...
```bash
0 2 * * * /path/to/scripts/backup-db.sh >> /var/log/isurf-backup.log 2>&1
```
