...
The React Native mobile app is the **primary user interface** for iSurf. It receives sessions from the watch, displays analytics, manages user accounts, and orchestrates sync with the backend.
...
```bash
npx react-native@latest init iSurfMobile \
...
import { SessionSummary, SessionMetadata } from '@isurf/shared-types';
...
import { Session } from '@isurf/shared-types';
...
import { create } from 'zustand';
import { User } from '@isurf/shared-types';
...
```objc
// ios/iSurf/WatchConnectivity/WatchConnectivityBridge.m
...
```swift
// ios/iSurf/WatchConnectivity/WatchConnectivityBridge.swift
import Foundation
