...
class SessionManager: ObservableObject {
    ...
    private let networkMonitorQueue = DispatchQueue(label: "com.spoteq.network-monitor")
        ...
        if let config = v16Config {
            sessionLogger.logEvent(
                "V16 engine=v\(JumpEngineV16.version) config pop=\(config.popMinG)g cluster=\(config.popClusterSec)s "
                    ...
                    + "heightPostRoll=\(config.heightPostRollSec)s "
                    + "heightPostRollMinAir=\(config.heightPostRollMinAirSec)s "
                    + "heightPostRollMin=\(config.heightPostRollMinM)m "
                    + "heightPostRollFloor=\(config.heightPostRollFloorSec)s "
                    + "heightCal=\(config.heightCalSlope)x+\(config.heightCalOffsetM)m "
