...
| `SPOTEQ Watch App/Services/JumpDetectorV14.swift` (ואותו ל-V15) | להוסיף init overload שמקבל `V14Config` מוזרק, לצד `makeConfigFromSettings()` הקיים ל-live | לנתק את replay מ-`UserDefaults.standard` (סעיף 5/17) | **בינוני** — קובץ production, גם אם live path לא משתנה | production init ללא ארגומנט נשאר זהה | E2E self-test קיים (`EngineE2ESelfTest`) חייב לעבור ללא שינוי |
