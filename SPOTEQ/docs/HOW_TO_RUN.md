# 🚀 How to Run the SPOTEQ Watch App on Your Mac
...
1. In Xcode, **delete** the auto-generated files:
   - `ContentView.swift`
   - `SPOTEQ_Watch_AppApp.swift` (or similar)

2. **Right-click** the project in navigator → **Add Files to "SPOTEQ"...**

3. Select the **entire `SPOTEQ` folder** from Finder

4. In the dialog:
   - ✅ Check "Copy items if needed"
   - ✅ Create groups (not folder references)
   - ✅ Add to targets: SPOTEQ
   - Click **Add**
...
| HealthKit errors | Settings → Health → Data Access → SPOTEQ → Enable all |
...
- Device: Settings → Privacy → Location → SPOTEQ → While Using
