...
final class JumpDetectorV16: JumpDetecting {
    ...
    /// Keys present in UserDefaults on the last makeConfigFromSettings() call.
    /// A stored override SILENTLY defeats a recalibration — 16.1 moved
    /// heightOffsetM 1.50 -> 1.43 and minReportM 1.5 -> 1.4, and a value left
    /// behind by 16.0 field work would keep the old figure with nothing on
    /// screen to say so. Nothing in this app writes these keys, so they can
    /// only arrive from a debugger or a companion build; that makes them rare
    /// AND easy to forget. They are surfaced in the reset log instead of being
    /// version-namespaced: renaming per release would silently discard a
    /// deliberate override too, and would need redoing every version.
    private(set) static var activeOverrides: [String] = []

    static func makeConfigFromSettings() -> V16Config {
        let defaults = UserDefaults.standard
        let base = V16Config()
        var cfg = base
        activeOverrides = [V16Settings.minReportM, V16Settings.popMinG,
                           V16Settings.minLiftPlateauSec, V16Settings.heightScale,
                           V16Settings.heightOffsetM]
    ...
    func reset(mode: DetectionMode) {
        ...
        if !Self.activeOverrides.isEmpty {
            logEvent("⚠️ v16 UserDefaults OVERRIDES ACTIVE: \(Self.activeOverrides.joined(separator: ", ")) "
                + "— these replace the 16.1 calibration defaults. Delete the keys to restore them.")
        }
    }
    ...
    func processSample(_ sample: IMUSample) {
        // NOTE: no `sampleCount` here. V8-V15 keep one to throttle a per-sample
        // log line (`sampleCount % 5 == 0`); V16 does not emit that line (see the
        // note at the end of this method), so the counter was never read — while
        // being incremented on the sensor thread and zeroed inside engineQueue.
        // A data race on a value nobody consumes. Removed rather than locked.
    ...
    private func makeJump(from result: V16Jump) -> Jump {
        ...
        // airtimeSec is LOW CONFIDENCE (see JumpEngineV16 header) and may be nil
        // when the descent was never arrested — 2 of 19 reference goldens are
        // like that, and so is a shore-side stray.
        //
        // ⚠️ Jump.airtime is a non-optional Double, so "unknown" has to be
        // encoded as 0 and endTime == startTime. THE UI MUST RENDER THAT AS
        // "—", NOT AS "0.0 s". The same coercion in the analysis dashboard was
        // silently turning an unknown airtime into a confident-looking 0.34 s
        // reading for a 1.8 m emission — shorter than free fall from that
        // height, so obviously impossible, yet displayed as a measurement.
        // Do not filter on airtime < 1 s either: no real jump in the reference
        // set flies under 2.40 s, so such a filter only ever removes the 0
        // sentinel, and 2 of the 5 such emissions are genuine jumps.
