...
public final class JumpEngineV16 {
    ...
    public func addIMU(t: TimeInterval,
        ...
        if load >= cfg.popMinG {
            if var last = pending.last, t - last.t0 < cfg.popClusterSec {
                ...
                onDebug(t, "POP t0=\(fmt(t)) yank=\(fmt(load))g")
            }
        }
    ...
    private func liftBins(from: TimeInterval, to: TimeInterval) -> Bins? {
        ...
        for i in ringHead..<ring.count {
            ...
            if s.az.isFinite {
                azRaw[k] += s.az
                attCnt[k] += 1
                totalAttitudeSamples += 1
            }
            if s.load <= cfg.floatLoadG { flRaw[k] += 1 }
            cnt[k] += 1
        }
        guard totalAttitudeSamples > 0 else { return nil }
    ...
    private static func boxSmooth(_ src: [Double], halfWidth: Int) -> [Double] {
        ...
        var sum = 0.0
        var finiteCount = 0
        for i in 0...min(n - 1, halfWidth) where src[i].isFinite {
            sum += src[i]
            finiteCount += 1
        }
        for i in 0..<n {
            // Keep the missing bin itself as NaN so a shelf cannot cross an
            // attitude hole, but do not let that NaN poison every subsequent
            // rolling sum after valid attitude resumes.
            if src[i].isFinite, finiteCount > 0 {
                out[i] = sum / Double(finiteCount)
            }

            let lo = i - halfWidth
            let hi = i + halfWidth
            if hi + 1 < n, src[hi + 1].isFinite {
                sum += src[hi + 1]
                finiteCount += 1
            }
            if lo >= 0, src[lo].isFinite {
                sum -= src[lo]
                finiteCount -= 1
            }
        }
