...
export interface V16Config {
  ...
  /** How far BEFORE t0 the height integral starts (s). See the note at the
   *  pre-roll. Height window only; detection and airtime are untouched. */
  heightPreRollSec: number;
  /** How far AFTER the resolved landing the height integral runs, on jumps whose
   *  airtime reaches heightPostRollMinAirSec (s). See the note at the post-roll. */
  heightPostRollSec: number;
  /** Airtime at or above which the post-roll applies (s). One of two triggers —
   *  see heightPostRollMinM for why it takes both. */
  heightPostRollMinAirSec: number;
  /** First-pass height at or above which the post-roll applies (m). The OTHER
   *  trigger: the two failure modes are mirror images, so neither gate alone is
   *  enough. See the note at the post-roll. */
  heightPostRollMinM: number;
  /** Floor on the flight window before the post-roll is added (s). A landing
   *  resolved this early on a big jump is not believed. */
  heightPostRollFloorSec: number;
  /** Reference calibration applied to the reported height, the single place
   *  where the measurement is mapped onto whatever counts as truth. See the
   *  note at the assignment. 1.0 / 0.0 targets Surfr as displayed and is a
   *  measured optimum, not an unset default. */
  heightCalSlope: number;
  /** Offset of the reference calibration (m). See heightCalSlope. */
  heightCalOffsetM: number;
...
export const V16_DEFAULTS: V16Config = {
  ...
  strongShelfSec: 1.05,
  heightPreRollSec: 0.5,
  heightPostRollSec: 0.8,
  heightPostRollMinAirSec: 4.0,
  heightPostRollMinM: 3.5,
  heightPostRollFloorSec: 5.0,
  // Target = Surfr as displayed. Identity is the measured optimum; see the note
  // at the calibration. For videogrammetric truth use 0.8025 / +0.7309 — but
  // only against our own video measurement.
  heightCalSlope: 1.0,
  heightCalOffsetM: 0.0,
...
export class JumpEngineV16 {
  ...
  private evaluate(c: Candidate, now: number, forced = true): boolean {
    ...
    // TAKE-OFF PRE-ROLL. t0 is the POP — the strongest sample of the take-off
    // burst — and the rider is already rising by then. Starting the integral
    // there truncates the first part of the ascent, and the signature is a
    // NEGATIVE height bias, which both references show independently: -0.118 m
    // against Surfr on Gavri's 15 second-precision rows, -0.210 m against HOOLAN
    // on 287 + smallLog.
    //
    // Sweeping the window start traces a clean U on each reference separately.
    // 0.3 s shipped in 16.5 on six logs; the Kineret session (41 Surfr goldens
    // at SECOND precision, 1.65-7.09 m) moved the weighted minimum to 0.5 s
    // across all eight:
    //
    //     pre    0.3     0.4     0.5     0.6     0.8
    //     MAE   0.458   0.444   0.442   0.447   0.458      (135 goldens)
    //
    // Adaptive pre-roll was tested in BOTH directions and both lose to the
    // constant: proportional to the shelf (0.595 vs 0.579), proportional to the
    // height (0.585), and inversely proportional to the shelf, which the Kineret
    // shelf-band biases seemed to ask for (cross-validated 0.565 vs 0.566).
    //
    // Applied to the HEIGHT WINDOW ONLY. Detection, airtime and distance still
    // use t0 unchanged — the same decoupled-anchor pattern as apexAnchorSec. The
    // free-fall window is exempt: it bounds a thrown watch exactly, and widening
    // it would re-introduce the error it was added to remove.
    //
    // Feeding the corroborated height rather than the raw one to the short-shelf
    // gate was tried and is WORSE: it costs V142 a real jump (4/5 -> 3/5). The
    // gate wants the same number the rider is shown.
    //
    // Known cost, one jump: on smallLog two candidates 2.4 s apart sit within
    // dedupSec, and the pre-roll flips which is "stronger" by 2 cm (1.61 vs
    // 1.59 m). The wrong one wins and its distance goes 19 -> 36 m against an
    // 18.2 m golden, which alone moves that log's distance MAE 4.50 -> 5.82 m.
    // It is a dedup coin-flip amplified by one outlier, not a systematic effect:
    // log 287's distance is unchanged at 3.77 m.
    if (!ballistic && winB != null) winA = t0 - this.cfg.heightPreRollSec;
    // LANDING POST-ROLL, 16.6. The mirror of the pre-roll, and the same argument:
    // an endpoint anchor is only sound where the rider is PROVABLY at water
    // level. A kite jump is not ballistic — the kite carries the rider, so the
    // descent is long and shallow, and the descent-arrest rule closes early on
    // it (airtime bias -0.41 s on Kineret's jumps at or above 4 m). Anchoring
    // z(T)=0 while the rider is still descending drags the apex down by
    // (t_apex/T) times the height still remaining, which is exactly the observed
    // signature: bias -0.08 m at 3.5-4.5 m growing to -1.09 m at 6.5-8 m.
    //
    // Running the window PAST the landing is safe because after touchdown the
    // trace is flat at water level and the chord absorbs it. Running it SHORT is
    // not: ending at the landing impact instead doubles the error (0.574 ->
    // 1.332), which also rules out the impact spike corrupting W(T).
    //
    // TWO TRIGGERS, BECAUSE THE FAILURE MODE HAS TWO FACES (16.7). An early
    // landing shows up either as a short airtime or as a low height, and which
    // one you see depends on where in the descent the arrest rule fired. The two
    // worst jumps in the Kineret session are exact mirror images:
    //
    //     Surfr 5.77 m / 5.15 s   ->   ours 2.69 m / 6.34 s   long air, LOW h
    //     Surfr 7.09 m / 5.79 s   ->   ours 5.38 m / 3.90 s   big h, SHORT air
    //
    // 16.6 gated on airtime alone and caught the first while the second — the
    // second-highest jump of the session — missed the 4.0 s gate by 0.10 s and
    // was left uncorrected. A height gate alone fails the other way. So the gate
    // is a DISJUNCTION, and the window also gets a FLOOR: a landing resolved
    // before heightPostRollFloorSec on a jump this size is not believed.
    //
    //                                  Surfr >=4 m  Kineret >=4 m  HOOLAN  jump
    //     16.5 (pre 0.3, no post)         0.706         0.628       0.207  5.37
    //     16.6 (airtime gate only)        0.588         0.528       0.226  5.44
    //     16.7 (this)                     0.569         0.488       0.222  6.31
    //     ungated, no gate at all         0.559         0.488       0.235  6.31
    //
    // The last column is that 7.09 m jump. The ungated version reaches the same
    // place but costs Gavri 34 % (0.522 -> 0.654) — a session of 2.4-3.1 m jumps
    // whose landings are already right. The disjunction gets the big-air gain
    // for 0.023 m of Gavri and IMPROVES HOOLAN (0.226 -> 0.222).
    //
    // Cross-validated by holding out a whole Surfr session: 0.706 -> 0.586 on
    // jumps at or above 4 m, with all three folds selecting a post-roll.
    //
    // The engine is STREAMING, so the samples the post-roll needs do not exist
    // at the landing instant. Measured lookahead past the landing at this point:
    // median 0.60 s on the candidates the gate selects, 0.00 s on smallLog.
    // Applying the post-roll without waiting makes flightHeight() return null on
    // its gap guard, the height silently falls back to the matched filter, and
    // the suite degrades badly (pooled 0.461 -> 0.629, Yaniv's extras 6 -> 13).
    // So WAIT for the window, exactly as the code above waits for the landing —
    // and at the evalDelaySec deadline take whatever has arrived rather than
    // overrun the buffer.
    if (!ballistic && winB != null && this.cfg.heightPostRollSec > 0) {
      const air = winB - t0;
      // First pass over the resolved flight. This is the number the airtime gate
      // cannot see, and it costs one extra integration over a window we already
      // hold in the ring.
      const h1 = this.cfg.heightFromFlight ? this.flightHeight(winA, winB) : null;
      const trigger = air >= this.cfg.heightPostRollMinAirSec
        || (h1 != null && h1 >= this.cfg.heightPostRollMinM);
      if (trigger) {
        const want = t0 + Math.max(air, this.cfg.heightPostRollFloorSec)
          + this.cfg.heightPostRollSec;
        const last = this.ring[this.ring.length - 1];
        if (last != null && last.t < want && !forced) return false;
        winB = last != null ? Math.min(want, last.t) : want;
      }
    }
    ...
    // ── REFERENCE CALIBRATION (16.6). One explicit place where the measurement
    // is mapped onto whatever we have decided counts as truth, so that decision
    // is a pair of numbers and not scattered through the estimator.
    //
    //   reported = heightCalSlope * measured + heightCalOffsetM
    //
    // CURRENT TARGET: Surfr as displayed. The calibration is the IDENTITY, and
    // that is a measured result, not a default left unset. Across 95 paired
    // jumps in three Surfr sessions our raw bias is already +0.006 m, so there
    // is no offset left to remove, and every slope tested made agreement WORSE
    // because it amplifies our scatter faster than it corrects a bias that is
    // already zero:
    //
    ...
    //     hinge fitted, session held out    0.593      27 %   (k < 0, shrinks
    //                                                          big jumps — the
    //                                                          wrong direction)
    //
    // THE OTHER TARGET, ready to switch to. Videogrammetry (Sensors 2021,
    // 21(24):8353; four shore cameras, reference accuracy 0.03-0.09 m, 20 jumps
    // 3.07-7.30 m, targets on the board) measured Surfr against ground truth:
    //
    //     truth = 0.8025 * surfr + 0.7309     R^2 0.961, residual 0.26 m rms
    //
    // Surfr overestimates 15 of 20 jumps there, and the bias grows with height:
    // +0.06 m at 3-4 m, +0.15 at 4-5, +0.19 at 5-6, and +0.73 m (11.4 %) at
    // 6-8 m. Our own numbers sit closer to that calibrated truth than to Surfr
    // as displayed (Kineret MAE 0.527 -> 0.476), and against it V16.5 was almost
    // unbiased (-0.076 m) while V16.6 overshoots (+0.144 m).
    //
    // That study used an iPhone SE BOLTED TO THE BOARD in 2021; the sessions we
    // validate against are Surfr.AI on a wrist watch in 2026, and Surfr state
    // they rebuilt the algorithm after moving to wrist and chest. The transform
    // is recorded here because it is the only independent ground truth that
    // exists, NOT because it is known to apply to the current app. Switch to it
    // only against our own video measurement.
    const rawHeightM = flightH ?? (this.cfg.heightScale * apex + this.cfg.heightOffsetM);
    const heightM = this.cfg.heightCalSlope * rawHeightM + this.cfg.heightCalOffsetM;
