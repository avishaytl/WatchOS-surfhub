...
export class JumpEngineV16 {
  ...
  private evaluate(c: Candidate, now: number, forced = true): boolean {
    ...
    // reference separately, and the weighted minimum sits at -0.3 s:
    //
    ...
    //     -0.9     0.325  +0.295     0.227 +0.083  0.261
    //
    // Two independent references agreeing on the direction AND the magnitude is
    // what separates this from fitting one of them. Shifting the LANDING does
    // almost nothing by comparison (0.213-0.219 across +-0.4 s), so the whole
    // error lives at the take-off end.
    //
