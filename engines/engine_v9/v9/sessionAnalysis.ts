...
import { jumpTrajectoryView, type JumpTrajectoryView } from './trajectoryV9.ts';
...
export function analyseSession(
  ...
  const jumps = detectJumpsV4(samples, params);
...
export function analyseSessionV7(
  ...
  const jumps: JumpEvent[] = v7.map((r) => {
    ...
    return toJumpEventV7(r, anchor, params.displayedAirtimeScale);
  });
...
/**
 * v8 session metrics — the BARO-CENTRIC kite engine (KitesurfJumpEngineV8). Kite
 * jump HEIGHT comes from the barometer (the wrist accelerometer does not contain
 * the smooth canopy-borne rise — see JUMP_V8_PLAN.md). Same SessionMetrics shape
 * as v7 plus the rich per-jump v8 results, exposed under both `v7` (for the
 * version-agnostic dashboard diagnostics column) and `v8`.
 */
export function analyseSessionV8(
  samples: SensorSample[],
  params: JumpEngineParams = DEFAULT_JUMP_PARAMS,
): SessionMetrics & { v7: JumpResult[]; v8: JumpResult[] } {
  const v8 = detectJumps(samples, params);
  ...
  const jumps: JumpEvent[] = v8.map((r) => {
    ...
    if (tMs != null) {
      let best = Infinity;
      for (const s of samples) { const d = Math.abs(s.t - tMs); if (d < best) { best = d; anchor = s; } }
    }
    return toJumpEvent(r, anchor);
  });
...
/**
 * analyseSessionV9 — the V9 architecture: identical metrics to analyseSessionV8,
 * but produced by REPLAYING the log through the on-watch periodic scanner (cyclic
 * ring buffer, 5 s ticks, eviction + ripeness + dedup) instead of one idealised
 * whole-log pass. V9 = the V8 baro-centric engine driven the way the watch runs
 * it in the field, so its differences from analyseSessionV8 (whole-log) are
 * exactly the real-world artefacts to debug.
 */
export function analyseSessionV9(
  samples: SensorSample[],
  params: JumpEngineParams = DEFAULT_JUMP_PARAMS,
  cfg: ScanConfigV9 = DEFAULT_SCAN_CONFIG,
): SessionMetrics & { v7: JumpResult[]; v8: JumpResult[]; scan: ReplayScanResult; trajectories: JumpTrajectoryView[] } {
  const scan = replayScan(samples, params, cfg);
  // canonical jumps = the FINAL (stable, accurate) emissions, chronological.
  const v8 = scan.finals
    .map((e) => e.jump)
    .sort((a, b) => (a.takeoffTimeMs ?? 0) - (b.takeoffTimeMs ?? 0));
  // per-jump trajectory + sensors (for the dashboard graph) — uses the engine's
  // accurate altitude (peaks at the real height), centred on the apex, ≥0.
  const altSeries = samples.length ? baroAltitudeSeries(samples, params) : [];
  const trajectories = v8.map((j) => jumpTrajectoryView(samples, altSeries, j));
