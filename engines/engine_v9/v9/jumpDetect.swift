...
//
//  ALGORITHM v9 — single source of truth is core/jumpScanV9.ts (TypeScript).
...
//
//  NATIVE WIRING: feed every CoreMotion/GPS sample to addSample(); call scan(now)
//  on a `tickSec` Timer → drive the HUD by `stage`+`id` (provisional shows, final
//  replaces it); call flush(now) once at session end.
//
...
enum ScanStage: String { case provisional, final_ = "final" }
...
private func estLandingMs(_ j: JumpResult) -> Double {
    (j.takeoffTimeMs ?? 0) + j.airTimeSec * 1000
}
private func jumpArcLo(_ j: JumpResult) -> Double { j.takeoffTimeMs ?? 0 }
private func jumpArcHi(_ j: JumpResult) -> Double { estLandingMs(j) }
private func arcsOverlap(_ aLo: Double, _ aHi: Double, _ bLo: Double, _ bHi: Double, _ margin: Double) -> Bool {
...
private final class PendingJump {
    ...
    init(id: Int, jump: JumpResult, seenAtMs: Double) { self.id = id; self.jump = jump; self.seenAtMs = seenAtMs }
}

/// Stateful periodic scanner. One instance per session.
final class JumpScannerV9 {
    private var buf: [JumpSample] = []
    private var pending: [PendingJump] = []
    private var finalizedArcs: [(lo: Double, hi: Double)] = []  // dedup across eviction
    private var nextId = 1
    private let params: JumpEngineParams
    private let cfg: ScanConfigV9
    private var lastTickMs = -Double.infinity

    init(_ params: JumpEngineParams = .default, _ cfg: ScanConfigV9 = .default) {
        self.params = params
        self.cfg = cfg
    }

    func addSample(_ s: JumpSample) { buf.append(s) }
    ...
    func scan(_ nowMs: Double) -> [ScanEmission] {
        ...
        for p in pending {
            ...
            if seenNow && !p.finalEmitted && age >= finMs {
                p.finalEmitted = true
                finalizedArcs.append((jumpArcLo(p.jump), jumpArcHi(p.jump)))
        ...
        pending = pending.filter { p in p.finalEmitted ? jumpArcHi(p.jump) >= winLo : nowMs - p.seenAtMs <= dropGapMs }
    ...
    func flush(_ nowMs: Double) -> [ScanEmission] {
        _ = track(nowMs)
        var out: [ScanEmission] = []
        for p in pending {
            if !p.provisionalEmitted { p.provisionalEmitted = true; out.append(emit(p, .provisional, nowMs, true)) }
            if !p.finalEmitted { p.finalEmitted = true; finalizedArcs.append((jumpArcLo(p.jump), jumpArcHi(p.jump))); out.append(emit(p, .final_, nowMs, true)) }
        }
        return out
    }
    ...
    private func track(_ nowMs: Double) -> Int {
        ...
        for j in KitesurfJumpEngineV8.detectJumps(window, params) {
            ...
            let aLo = jumpArcLo(j), aHi = jumpArcHi(j)
            if finalizedArcs.contains(where: { arcsOverlap($0.lo, $0.hi, aLo, aHi, marginMs) }) { continue }
            if let p = pending.first(where: { arcsOverlap(jumpArcLo($0.jump), jumpArcHi($0.jump), aLo, aHi, marginMs) }) {
                if !p.finalEmitted { p.jump = j }
                p.seenAtMs = nowMs
            } else {
                pending.append(PendingJump(id: nextId, jump: j, seenAtMs: nowMs)); nextId += 1
            }
        }
        return lo
    }

    private func windowSince(_ winLo: Double) -> ([JumpSample], Int) {
        var lo = 0
        while lo < buf.count && buf[lo].t < winLo { lo += 1 }
        return (lo > 0 ? Array(buf[lo...]) : buf, lo)
    }

    private func emit(_ p: PendingJump, _ stage: ScanStage, _ nowMs: Double, _ viaFlush: Bool) -> ScanEmission {
        ScanEmission(jump: p.jump, stage: stage, id: p.id, emittedAtMs: nowMs,
                     emitLatencySec: ((nowMs - estLandingMs(p.jump)) / 1000 * 10).rounded() / 10, viaFlush: viaFlush)
    }
}
